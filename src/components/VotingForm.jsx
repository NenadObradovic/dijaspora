...
const VotingForm = ({ onSubmit }) => {
	...
	return (
		<div>
			<div className="tabs">
				<button onClick={() => setActiveTab('check')} className={activeTab === 'check' ? 'active' : ''}>{t('check_tab')}</button>
				<button onClick={() => setActiveTab('personal-data')} className={activeTab === 'personal-data' ? 'active' : ''}>{t('personal_data_tab')}</button>
				<button onClick={() => setActiveTab('signature')} className={activeTab === 'signature' ? 'active' : ''}>{t('signature_tab')}</button>
			</div>
			<form onSubmit={handleSubmit}>
				{activeTab === 'check' && (
					<>
						<div>
							...
							<a href={JEDINSTVEN_BIRACKI_SPISAK_URL} target="_blank">{t('check_list_link')}</a>
						</div>
						<div>
							<label form="generateVoter">{t('generate_request_for_list')}</label>
				...
				{activeTab === 'personal-data' && (
					<>
						<div>
							<input type="text" name="full_name" placeholder={t('full_name')} value={data.full_name} onChange={handleChange} required/>
							<span>{t('full_name_description')}</span>
						</div>
						<div>
							<input type="text" name="parent_name" placeholder={t('parent_name')} value={data.parent_name} onChange={handleChange} required />
							<span>{t('parent_name_description')}</span>
							{handleFieldError( 'parent_name' )}
						</div>
						<div>
							<input type="text" name="jmbg" placeholder={t('jmbg')} value={data.jmbg} onChange={handleChange} min={13} max={13} required />
							<span>{t('jmbg_description')}</span>
							{handleFieldError( 'jmbg' )}
						</div>
						<div>
							<input type="text" name="address" placeholder={t('address')} value={data.address} onChange={handleChange} required />
							<span>{t('address_description')}</span>
							{handleFieldError( 'address' )}
						</div>
						<div>
							<input type="text" name="address_abroad" placeholder={t('address_abroad')} value={data.address_abroad} onChange={handleChange} required />
